import { prisma, inParams } from "./prisma";
...
// Socle de génération de la synthèse Lia, partagé par la route POST /api/topo
// (déclenchement manuel / regénération) et par la publication d'une journée
// (api/admin/publish, génération automatique). Aucune logique de HTTP ici.
...
// Génère (ou renvoie le cache de) la synthèse Lia pour une ligue, sur la journée
// courante. `force` bypass le cache pour regénérer. Lève TopoError(404) si la
// ligue est inconnue, TopoError(503) si tous les modèles sont indisponibles.
export async function generateTopo(slug: string, force = false): Promise<TopoResult> {
  const league = await getLeagueBySlug(slug);
  if (!league) {
    throw new TopoError("Ligue non trouvée", 404);
  }
  ...
  // If `force === true`, always regenerate (bypass cache)
...
Écris UNIQUEMENT le texte de la synthèse, rien d'autre.`;
  ...
  if (!text) {
    throw new TopoError("Tous les modèles IA sont indisponibles. Réessayez dans quelques minutes.", 503);
  }
  ...
  return { topo: text, matchday: currentDay, cached: false, isProvisional: isIncomplete };
}
